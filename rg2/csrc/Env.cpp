...
public:
  explicit ENVIRONMENT(const std::string &resourceDir, const Yaml::Node &cfg,
                       bool visualizable)
      : UnitEnv(resourceDir, cfg), visualizable_(visualizable),
        normDist_(0, 1) {
    ...
    nJoints_ = gvDim_ - 6;   // initial actionable joints
    ...
    // gcInit_ << 0, 0, 0.50, 1.0, 0.0, 0.0, 0.0, 0.03, 0.4, -0.8, -0.03, 0.4,
  ...
  void setInitConstants(Eigen::VectorXd gcInit, Eigen::VectorXd gvInit,
                        Eigen::VectorXd actionMean, Eigen::VectorXd actionStd,
                        Eigen::VectorXd pGain, Eigen::VectorXd dGain) {
...
private:
  int gcDim_, gvDim_, nJoints_, nActionableJoints_;
